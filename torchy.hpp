...
public:
    ...
    Tensor(std::vector<int> size, std::vector<float> data, std::vector<std::shared_ptr<Tensor>> children) : size_(size), data_(data), grad_(_product(size), 0), children_(children), op_('?') {};

    std::shared_ptr<Tensor> get_shared() {
        return this->shared_from_this();
    }

    void size() { for (auto x : size_) std::cout << x << std::endl; }
    void data() { for (auto x : data_) std::cout << x << std::endl; }
    void grad() { for (auto x : grad_) std::cout << x << std::endl; }
    void children() { for (auto x : children_) std::cout << x << std::endl; }
...
std::shared_ptr<Tensor> operator+(std::shared_ptr<Tensor> lhs, std::shared_ptr<Tensor> rhs) {
    ...
    auto result = std::make_shared<Tensor>(lhs.get()->size_, result_data, children);
    return result;
}

std::shared_ptr<Tensor> tensor(std::initializer_list<int> size, std::initializer_list<float> data, std::vector<std::shared_ptr<Tensor>> children = {}) {
    Tensor t(size, data);
