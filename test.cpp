...
TEST(Torch, ScalarMultiOperatorSubtest0) {
  Tensor<float> a({1}, {4.20}, true);
  Tensor<float> b({1}, {13.37}, true);
  auto c = (b / a) + b;
  c.backward();
...
TEST(Torch, ScalarMultiOperatorSubtest1) {
  Tensor<float> a({1}, {4.20}, true);
  Tensor<float> b({1}, {13.37}, true);
  auto c = (a * b) - (b);
  c.backward();
...
TEST(Torch, ScalarMultiOperatorSubtest4) {
...
TEST(Torch, ScalarMultiOperatorSubtest5) {
  ...
  torch::Tensor l_t = ((c_t / d_t) + e_t);
  l_t.backward();
  auto c_v_t = tensorToVector<float>(c_t.grad());
  auto d_v_t = tensorToVector<float>(d_t.grad());
  auto e_v_t = tensorToVector<float>(e_t.grad());
  auto l_v_t = tensorToVector<float>(l_t.data());
...
TEST(Torch, ScalarMultiOperatorAlterationAddition) {
  Tensor<float> a({1}, {4.20}, true);
  auto c = a + a + a;
  c.backward();
  auto a_v = a.grad();
...
TEST(Torch, ScalarMultiOperatorAlterationSubtraction) {
  Tensor<float> a({1}, {4.20}, true);
  auto c = a - a - a;
  c.backward();
  auto a_v = a.grad();
...
TEST(Torch, ScalarMultiOperatorAlterationMultiplicationBackets0) {
  Tensor<float> a({1}, {4.20}, true);
  auto c = ((a * a) * a);
  c.backward();
  auto a_v = a.grad();
...
TEST(Torch, ScalarMultiOperatorAlterationMultiplicationBackets1) {
  Tensor<float> a({1}, {4.20}, true);
  auto c = (a * (a * a));
  c.backward();
  auto a_v = a.grad();
...
TEST(Torch, ScalarMultiOperatorAlterationMultiplicationZero) {
  Tensor<float> a({1}, {0.0}, true);
  auto c = a * a * a;
  c.backward();
  auto a_v = a.grad();
  auto c_v = c.data();

  torch::Tensor a_t = torch::tensor({0.0}, torch::requires_grad(true));
  torch::Tensor c_t = a_t * a_t * a_t;
  c_t.backward();
  auto a_v_t = tensorToVector<float>(a_t.grad());
  auto c_v_t = tensorToVector<float>(c_t.data());
...
TEST(Torch, ScalarMultiOperatorAlterationMultiplicationOne) {
  Tensor<float> a({1}, {1.0}, true);
  auto c = a * a * a;
  c.backward();
  auto a_v = a.grad();
  auto c_v = c.data();

  torch::Tensor a_t = torch::tensor({1.0}, torch::requires_grad(true));
  torch::Tensor c_t = a_t * a_t * a_t;
  c_t.backward();
  auto a_v_t = tensorToVector<float>(a_t.grad());
  auto c_v_t = tensorToVector<float>(c_t.data());
...
TEST(Torch, ScalarMultiOperatorAlterationMultiplicationTwoUnique) {
  Tensor<float> a({1}, {2.0}, true);
  Tensor<float> b({1}, {2.0}, true);
  Tensor<float> c({1}, {2.0}, true);
  auto l = a * b * c;
  l.backward();
  auto a_v = a.grad();
  auto b_v = b.grad();
  ...
  torch::Tensor a_t = torch::tensor({2.0}, torch::requires_grad(true));
  torch::Tensor b_t = torch::tensor({2.0}, torch::requires_grad(true));
  torch::Tensor c_t = torch::tensor({2.0}, torch::requires_grad(true));
  torch::Tensor l_t = a_t * b_t * c_t;
  l_t.backward();
  auto a_v_t = tensorToVector<float>(a_t.grad());
  auto b_v_t = tensorToVector<float>(b_t.grad());
  auto c_v_t = tensorToVector<float>(c_t.grad());
